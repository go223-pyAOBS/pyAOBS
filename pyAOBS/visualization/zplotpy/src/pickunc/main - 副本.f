...
c
	 ...
	 +                  ibndps,freqlo,freqhi,izerop,npoles
c
...
670   continue
c
c     open(unit=15, file='../pickunc.in', status='old')
...
c
      write(6,45)
	  write(11,45)
45	  format(/'  pick    number   ave unc'/
     +                  '--------------------------'/)
c     
      do 510 i=1,npicko
         if(numunc(i).gt.0) uncave(i)=uncave(i)/float(numunc(i))
510   continue
c
      write(6,55)(pick(i),numunc(i),uncave(i),i=1,npicko)
      write(11,55)(pick(i),numunc(i),uncave(i),i=1,npicko)
55	  format(i6,i10,f10.4)
