...
c      Hilbert Transform 
c      
c      Author: Jonathas Maciel
c      E-mail: jonathassilvamaciel@gmail.com
c      Modified by Haibo Huang, 2023.11
c      ----------------------------------------
       parameter(pi=3.14159265359)
	   integer it,i,ih
       real  x(n),h(n),pi,soma,evelp,phase
       real buffer(-n:n),theta
...
21     continue
       ...
       do 31 it = 1,n       
          ...
          do 41 i=-n,n
             if (it.eq.i) goto 42
             soma = soma + buffer(i)/float(i-it)
c			 write(*,*)'soma=',soma
42			num=num+1
41        continue
            soma=soma/pi
			theta=soma/x(it)
            evelp=sqrt(x(it)**2+soma**2)
			phase=atan(theta)
c			write(*,*)'phase=',phase
          if(ih.eq.1) x(it) = phase
		  if(ih.eq.2) x(it) = evelp
31     continue
      return
      end
