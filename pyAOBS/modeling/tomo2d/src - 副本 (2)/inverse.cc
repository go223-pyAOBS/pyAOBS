...
#pragma omp for schedule(dynamic,1)
	    ...
	    std::vector< std::vector< std::pair<int,double> > > A_local_entries(nrcv_src);
	    std::vector<double> path_length_local(nrcv_src, 0.0);

	    auto add_kernel_local = [&](int ircv_local, const Array1d<Point2d>& cur_path){
		std::vector< std::pair<int,double> >& A_i = A_local_entries[ircv_local-1];
		...
		for (int i=1; i<=np+1; i++){
			...
			if (icell>0){
			    A_i.push_back(std::make_pair(jUL, rr*ss*dist));
			    A_i.push_back(std::make_pair(jLL, rr*s*dist));
			    A_i.push_back(std::make_pair(jLR, r*s*dist));
			    A_i.push_back(std::make_pair(jUR, r*ss*dist));
			}
		    }
		}
		...
		A_i.push_back(std::make_pair(nnodev+jL, com_factor*(x2-x)));
		A_i.push_back(std::make_pair(nnodev+jR, com_factor*(x-x1)));
		...
		std::vector< std::pair<int,double> >& entries = A_local_entries[ircv-1];
