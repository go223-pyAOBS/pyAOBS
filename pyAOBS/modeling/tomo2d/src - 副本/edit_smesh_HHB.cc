 ...
 * edit_smesh_HHB.cc
 *
 * usage: edit_smesh_HHB smesh_file -C<cmd>
 *
 *        -C<cmd>
 ...
 *                'b<k>/basefile' - set sub-Base velocity as vgrid * 1/<k> <Vp/Vs>
 *
 *        -Lvcorr.file
 *        -Uupperbound.file
 *
 ...
 * February 1999
 * Edited by Haibo Huang
 * Add prof1dbg and removeBG and -Bprof1dbg
 * Add kstart(i) for checkboard
 * Add -Cb<k>/basefile for converting Vp below basement to Vs
 * Add -CDabnormal_damp/normal_ndamp/boundary (Moho or Base)
 */
...
int main(int argc, char **argv)
{
    ...
    bool paste1dprof=false,removeBG=false;
    ...
    char *sfn, *pfn, *corrfn, *prof1dfn,*prof1dbg;
    ...
    bool subMohoVel=false,subBaseK=false,subbdDamp=false;
    double vsubmoho,ksubbase,damptop,dampbot;
	...
	if (argv[i][0] == '-'){
		...
		case 'm':
		    ...
		    break;
		case 'D':
		    if (sscanf(&argv[i][3], "%lf/%lf/%[^/]", &damptop,&dampbot,uboundfn)==3){
			subbdDamp = true;
		    }else{
			cerr << "invalid -CD option\n";
			err = true;
		    ...
		    break;			
		case 'b':
		    if (sscanf(&argv[i][3], "%lf/%[^/]", &ksubbase, uboundfn)==2){
			subBaseK = true;
		    }else{
			cerr << "invalid -Cb option\n";
			err = true;
		    }
		    break;			
		default:
	...
	for (int k=1; k<=nz; k++){
	    ...
	    s >> vel;
	    vgrid(i,k) = vel;
		if (subbdDamp) vgrid(i,k) = damptop;
	}
    }
    ...
    kstart = 1;
    if (getBound || subMohoVel || subBaseK){
	ubound_p = new Interface2d(uboundfn);
	for (int i=1; i<=nx; i++){
	    double boundz = ubound_p->z(xpos(i));
	    for (int k=1; k<=nz; k++){
		if (zpos(k)+topo(i) > boundz-0.1) break;
		kstart(i) = k;
	    }
	...
	for (int i=1; i<=nx; i++){
        ...
        Interface2d prof1d(prof1dbg); // this usage of interface1d class
	                              // is quite ad hoc (caution)
	  v_water=999;
	  v_air=999;
	for (int i=1; i<=nx; i++){
	    double z0 = zpos(kstart(i));
	    for (int k=1; k<kstart(i); k++){
  		vgrid(i,k) = 0; 
            }	    
	    for (int k=kstart(i); k<=nz; k++){
	        double z = zpos(k)-z0;
		double vval = prof1d.z(z);
		vgrid(i,k) = vgrid(i,k)-vval; 
            }
        }
    }else if (smooth){
	double Lh2 = Lh*Lh;
...
//cerr << zamp.back() << ' ' << zph.back() << '\n';
	...
	for (int i=1; i<=nx; i++){
	    for (int k=kstart(i); k<=nz; k++){
		vgrid(i,k) = vgrid(i,k)*1.0/ksubbase;
	    }
	}
	}	
