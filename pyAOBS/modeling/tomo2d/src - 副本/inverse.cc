...
TomographicInversion2d::TomographicInversion2d(SlownessMesh2d& m, const char* datafn,
    ...
    : smesh(m), graph(smesh,xorder,zorder),
      ...
      nrefl(0), nnoded(0), do_full_refl(false),do_convert(false),
      ...
      log_os_p(0), vmesh_os_p(0), grav_dws_osp(0), reflp(0), convp(0), dump_os_p(0)
{
...
void TomographicInversion2d::solve(int niter)
{
	...
	*log_os_p << "# LSQR " << LSQR_ATOL << endl;
	...
	if (parallel_src && iter==1){
	    cerr << "TomographicInversion2d:: parallel ray tracing enabled (source-wise, OMP)\n";
	}
#pragma omp parallel for if(parallel_src) schedule(dynamic,1) reduction(+:graph_time,bend_time)
	for (int isrc=1; isrc<=src.size(); isrc++){
	    ...
	    std::vector< std::map<int,double> > A_local(nrcv_src);
	    std::vector<double> path_length_local(nrcv_src, 0.0);

	    auto add_kernel_local = [&](int ircv_local, const Array1d<Point2d>& cur_path){
		std::map<int,double>& A_i = A_local[ircv_local-1];
		const int np = cur_path.size();
		const int nintp_local = betasp.numIntp();
		Array1d<const Point2d*> pp_local;
		Array1d<Point2d> Q_local(nintp_local);
		makeBSpoints(cur_path, pp_local);

		double path_len = 0.0;
		Index2d guess_index = smesh.nodeIndex(smesh.nearest(*pp_local(1)));
		for (int i=1; i<=np+1; i++){
		    int j1=i;
		    int j2=i+1;
		    int j3=i+2;
		    int j4=i+3;
		    betasp.interpolate(*pp_local(j1),*pp_local(j2),*pp_local(j3),*pp_local(j4),Q_local);

		    for (int j=2; j<=nintp_local; j++){
			Point2d midp = 0.5*(Q_local(j-1)+Q_local(j));
			double dist = Q_local(j).distance(Q_local(j-1));
			path_len += dist;
	    ...
	    auto add_kernel_refl_local = [&](int ircv_local, const Array1d<Point2d>& cur_path, int ir0, int ir1){
		std::map<int,double>& A_i = A_local[ircv_local-1];
		int jL, jR;
		...
		add_kernel_local(ircv_local, cur_path);
	    };
	    ...
	    //
	    Array1d<double> tmp_modelv_src;
	    if (do_full_refl){
		double pwater = 1.0/1.5;
		tmp_modelv_src.resize(modelv.size());
		tmp_modelv_src = modelv;
	    ...
	    if(do_convert){
		if (!parallel_src && verbose_level>=1) cerr << "test hhb do_convert\n";
        graph_local.solve_conv(src(isrc),*convp);
        }else graph_local.solve(src(isrc));
		...
		if (icode == 0){ // refraction
			...
			// with water velocity
			smesh.set(tmp_modelv_src);
		    }
		    if (!is_refl_solved){
			double start_t=inv_wall_seconds();
            if(do_convert){
            graph_local.solve_refl_conv(src(isrc),*reflp,*convp);
            }else graph_local.solve_refl(src(isrc), *reflp);
			double end_t=inv_wall_seconds();
				   ...
				   bool dv, double wdv, bool dd, double wdd)
{
    ...
    iterativeSolver_LSQR(B,total_data_vec,dmodel_total,LSQR_ATOL,lsqr_itermax,chi);
