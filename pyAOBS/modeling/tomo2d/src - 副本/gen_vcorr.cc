 ...
 * gen_vcorr.cc
 *
 * usage: gen_vcorr [ options ]
 *
 * <velocity setting options>
 *     [uniform gradient]
 *             -Aanorm_h/anorm_v/norm_h/norm_v -Bgradient
 *     [Zelt's v.in]
...
int main(int argc, char **argv)
{
    ...
    bool zeltdump=false;
    double v0, vgrad, xmax, zmax, dx, norm_h,norm_v,anorm_v,wcol=0.0;
	double anorm_h;
    int nx, nz, ilayer, imoho, itop,ibot;
    char *xfn, *zfn, *tfn, zeltfn[MaxStr], reflfn[MaxStr];
    char *zdfn; 
    double v_water = 1.50,depth,vh,vd;
    double v_air = 0.330;
	...
	if (argv[i][0] == '-'){
	    switch(argv[i][1]){
	    case 'A':
		if (sscanf(&argv[i][2], "%lf/%lf/%lf/%lf", &anorm_h,&anorm_v,&norm_h,&norm_v) != 3){
		    cerr << "invalid -A option;give abnormals/normals correlation lengths\n";
		}
		break;
	    case 'C':
    ...
    if (err) error("usage: gen_vcorr [ -options ]");
	...
	for (int k=1; k<=nz; k++){
		...
		vh = norm_h;
		if(depth > damptop(i) && depth < (dampbot(i)-0.10)) vh = anorm_h;
    }
	    cout << vh << " ";
	}
	cout << '\n';
    ...
    for (int i=1; i<=nx; i++){
        for (int k=1; k<=nz; k++){
            ...
            if (readZelt){
                ...
                depth=topo(i)+z(k)+eps;
                vd = norm_v;
                if(depth > damptop(i) && depth < (dampbot(i)-0.10)) vd = anorm_v;
            }
            cout << vd << " ";
        }
        cout << '\n';
