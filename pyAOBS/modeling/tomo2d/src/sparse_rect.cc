...
void SparseRectangular::columnNorm2(Array1d<double>& norm) const
{
    if (norm.size() != ncol){
	error("SparseRectangular::columnNorm2 - size mismatch");
    }
    for (int j=1; j<=ncol; j++) norm(j) = 0.0;
    for (int i=1; i<=nrow; i++){
	for (int k=first_k(i); k<first_k(i+1); k++){
	    const int j = index_j(k);
