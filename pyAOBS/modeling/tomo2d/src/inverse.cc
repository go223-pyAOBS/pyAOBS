...
void TomographicInversion2d::solve(int niter)
{
	...
	*log_os_p << "# smooth_dep " << smooth_depth << " "
	...
	if (damping_is_fixed){
	    *log_os_p << "# fixed_damping " << weight_d_v << " " << weight_d_d << '\n';
	    if (do_squeezing){
		*log_os_p << "# squeezing enabled wdv=" << weight_d_v << '\n';
	    }
	}else{
	...
	*log_os_p << "# LSQR " << LSQR_ATOL << endl;
	...
	double c2f_progress = (niter>1) ? double(iter-1)/double(niter-1) : 1.0;
	...
	if (enable_forward_reuse && has_prev_forward){
	    double dm2 = 0.0;
	    double m2 = 0.0;
