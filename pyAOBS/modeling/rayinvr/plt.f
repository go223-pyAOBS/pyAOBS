...
c
     ...
     +    sep=sep+orig+tmm
      ...
      ititle=0
      if(iaxlab.eq.1) then
        call axtick(xtmin,xtmax,xtmin,xtmax,ntickx,ndecix)
        call axtick(zmin,zmax,ztmin,ztmax,ntickz,ndeciz)
c       call axis(orig,sep+zmm,xtmin,xtmax,xmm,xscale,0.,-1,
...
c    +       ztmin,ztmax,ntickz,ndeciz,'DEPTH (m)',9,albht) 
        call axis(orig,sep+zmm,xtmin,xtmax,xmm,xscale,0.,-1,
...
c
        do 10 i=1,ncont
           ...
           if(nptsc.eq.1) then
             ...
             do 20 j=1,nptsc
                xcp(j)=(xm(i,j)-xtmin)/xscale+orig
...
c
        if(ifrbnd.eq.1) then
          do 110 i=1,nfrefl
             do 120 j=1,npfref(i)
                xcp(j)=(xfrefl(i,j)-xtmin)/xscale+orig
...
c
          do 30 i=1,nlayer
             if(nblk(i).gt.1) then
               do 40 j=1,nblk(i)-1
                  xcp(1)=(xbnd(i,j,2)-xtmin)/xscale+orig
                  xcp(2)=xcp(1)
...
c
          do 600 i=1,npbnd 
             x=xtmin+float(i-1)*xsinc
             xcp(i)=(x-xtmin)/xscale+orig
600       continue 
          do 610 i=1,nlayer+1
             do 620 j=1,npbnd
...
165             xp=xbnd(i,j,1)
                ...
                xp1=(xp-xtmin)/xscale+0.5*htt+orig
                xp=xbnd(i,j,2)
                z2=s(i,j,1)*xp+b(i,j,1)
                zp2=(z2-zmax)/zscale-1.5*htt+sep
                xp2=(xp-xtmin)/xscale-4.0*htt+orig
                xp=xbnd(i,j,1)
...
c
      do 180 i=1,npts
         x(i)=(xr(i+nskip)-xtmin)/xscale+orig
