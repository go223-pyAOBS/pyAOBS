...
c                 
     ...
     +        insmth(pncntr)
      character flag*1,tfile*80,vfile*80
c
      include 'rayinvr.com'                 
c                 
...
1       format(' '/' '/' ')
        ...
        open(unit=20, file=vfile, status='old')
        iunit=20
c        write(6,*) 'open the v.in file!'
      end if
c
...
c
      if(idata.ne.0.or.invr.eq.1.or.i2pt.ne.0) then  
        open(unit=26, file=tfile, status='old')
        isf=1
        nsf=0
        xshotc=0.
...
c
      write(6,*) 'calculate velocity model parameters'                 
...
65            format('***  shot#',i4,' ray code',f5.1,
     +               ' no rays traced  ***')
            ...
            if(amaxr.eq.aminr.and.ihdwf.ne.1) then
              if(nrayr.gt.1) then
c                write(11,665) ishotw(is),ray(i)
...
c            write(6,375) ishotw(is),ray(i),nrayr,flag
