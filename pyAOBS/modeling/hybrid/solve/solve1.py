...
def test_compute_L():
    ...
    # 初始化权重和参数
    lambda_h = 0.1  # 水平平滑权重
    lambda_v = 0.2  # 垂直平滑权重（稍大以保持垂直连续性）
    ...
    for iteration in range(max_iterations):
        ...
        if iteration > 0:
            if rms > rms_history[-2]:
                ...
                lambda_h = min(lambda_h * 1.2, 0.2)  # 增加水平平滑
                lambda_v = min(lambda_v * 1.2, 0.4)  # 增加垂直平滑
                logger.info(f"减小步长到: {step_length}, 增加平滑参数到: λh={lambda_h}, λv={lambda_v}")
            elif rms < rms_history[-2] * 0.95:  # 如果RMS显著改善
                step_length = min(step_length * 1.1, 0.8)
                lambda_h = max(lambda_h * 0.9, 0.05)  # 减小水平平滑
                lambda_v = max(lambda_v * 0.9, 0.1)  # 减小垂直平滑
                logger.info(f"增大步长到: {step_length}, 减小平滑参数到: λh={lambda_h}, λv={lambda_v}")
        
        # 更新阻尼因子
        current_damping = update_damping_weights(iteration, rms_history, 
                                              current_damping, base_damping)
    ...
    plt.tight_layout()
    plt.savefig('test_compute_L_inversion_old.png', dpi=300, bbox_inches='tight')
    plt.close()
